...
public class AuthService(IAuthRepository authRepository, IJwtService jwtService, ILogger<AuthService> logger, IEmailSender emailSender):IAuthService
{
    ...
    public async Task<AuthResponseDto?> RegisterAsync(RegisterRequestDto registerRequest)
    {
        ...
        logger.LogInformation("Created address: {addressId}", createdAddress.Id);
        ...
        var user = new User
        {
            ...
            DateOfBirth = registerRequest.DateOfBirth ?? DateOnly.FromDateTime(DateTime.UtcNow),
        ...
        try
        {
            var res = await emailSender.RegisterNotification(registerRequest);
            if(res) logger.LogInformation("Email notification sent on: {email}", createdUser.Email);
        }
        catch (Exception ex)
        {
            logger.LogWarning("Failed to send email notification: {error}", ex.Message);
        }
